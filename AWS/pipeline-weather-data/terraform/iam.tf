...
# ----------------------------------------------------------------------------------
# Note: This requires the IAM user specified in aws_iam_user_arn_by_snowflake to exist first.
# Create the user in AWS IAM before enabling this resource, or set enable_snowflake_integration = false
...
resource "aws_iam_role_policy_attachment" "snowflake_role_policy_attachment" {
  count      = var.enable_snowflake_integration ? 1 : 0
  role       = aws_iam_role.snowflake_service_role[0].id
  policy_arn = aws_iam_policy.snowflake_service_role_policy[0].arn
}
