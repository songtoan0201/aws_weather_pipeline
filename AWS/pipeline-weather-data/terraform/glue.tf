...
resource "aws_glue_catalog_table" "dim_locations" {
  ...
  storage_descriptor {
    location = "s3://${aws_s3_bucket.weather_datalake.id}/${var.pipeline_name}/warehouse/warehouse_weather_data.db/dim_locations/" 
  ...
  depends_on = [
    aws_glue_catalog_database.warehouse_weather_data,
    aws_s3_bucket.weather_datalake
  ] # Explicit dependency on the database and S3 bucket
}